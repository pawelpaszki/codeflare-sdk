        ...
        try:
            ...
            try:
                ...
                if cluster_state not in ["ready", "Ready"]:
                    print(f"⚠️  WARNING: Cluster state is '{cluster_state}', not 'ready'")
                    ...
                    for condition in conditions:
                        ...
                        print(f"  - {condition_type}: {condition_status} ({condition_reason})")
                
            except ApiException as e:
                if e.status == 404:
                    print(f"✗ ERROR: RayCluster '{CLUSTER_NAME}' not found in namespace '{NAMESPACE}'")
                    print("This suggests the pre-upgrade cluster creation test failed or didn't run.")
                    print("The UI test cannot proceed without an existing cluster.")
                    raise AssertionError(f"RayCluster '{CLUSTER_NAME}' does not exist in namespace '{NAMESPACE}'. "
            ...
            print(f"WARNING: Could not verify cluster existence via Kubernetes API: {e}")
            print("Proceeding with UI test, but this might be why the cluster is not visible.")
