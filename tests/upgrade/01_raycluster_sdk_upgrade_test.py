...
class TestMnistJobSubmit:
        ...
        try:
            ...
            if "oidcProviders" in spec and spec["oidcProviders"]:
                for provider in spec["oidcProviders"]:
                    ...
                    if ("keycloak" in issuer_url.lower() and ("rh-ods.com" in issuer_url or "qe.rh-ods.com" in issuer_url)) or "realms/openshift" in issuer_url:
    ...
    def assert_jobsubmit_withoutLogin(self, cluster):
        ...
        if is_byoidc_cluster:
            ...
            try:
                response = requests.get(dashboard_url, verify=False, allow_redirects=False)
                if response.status_code in [302, 401, 403]:
                    print(f"✓ Dashboard properly redirects unauthenticated access (status: {response.status_code})")
                    print("✓ BYOIDC authentication enforcement verified")
                    return
                else:
                    print(f"Warning: Dashboard returned unexpected status {response.status_code} for unauthenticated access")
    ...
    def assert_jobsubmit_withlogin(self, cluster):
        ...
        if is_byoidc_cluster:
            ...
            try:
                ...
                if cluster_details and hasattr(cluster_details, 'status') and cluster_details.status:
                    print(f"✓ Ray cluster is accessible and has status: {cluster_details.status}")
                    print("✓ BYOIDC authentication test passed - cluster is accessible via Kubernetes API")
                    return
                else:
                    raise RuntimeError("Cluster details could not be retrieved or cluster is not ready")
            except Exception as e:
                raise RuntimeError(f"Failed to verify cluster accessibility via Kubernetes API: {e}")
        ...
        try:
            ...
            try:
                ...
                print(f"Warning: Could not extract token from kubeconfig: {token_error}")
                header = {}
