...
class TestRayJobLifecycledCluster:
    ...
    def test_lifecycled_kueue_managed(self):
        ...
        try:
            ...
            assert job_running, f"RayJob {rayjob.name} failed to reach running state after retries"
    ...
    def _wait_for_job_running_with_retry(self, rayjob: RayJob, max_retries: int = 2) -> bool:
        """
        ...
        for attempt in range(max_retries + 1):
            try:
                print(f"Waiting for RayJob {rayjob.name} to reach running state (attempt {attempt + 1}/{max_retries + 1})")
                ...
                if self.job_api.wait_until_job_running(
                    ...
                    print(f"✓ RayJob {rayjob.name} reached running state on attempt {attempt + 1}")
                    return True
                else:
                    print(f"✗ RayJob {rayjob.name} failed to reach running state on attempt {attempt + 1}")
            ...
            if attempt < max_retries:
                ...
                try:
                    from support import verify_authentication_stability, setup_authentication
                    if not verify_authentication_stability():
                        print("Authentication stability check failed, re-initializing...")
                        setup_authentication()
                        print("Re-initialized authentication for retry")
                    else:
                        print("Authentication is stable, proceeding with retry")
                except Exception as auth_e:
                    print(f"Warning: Could not verify/re-initialize authentication: {auth_e}")
        
        print(f"✗ RayJob {rayjob.name} failed to reach running state after {max_retries + 1} attempts")
        return False
