...
class TestRayClusterSDKOauth:
    ...
    def _is_ray_cluster_auth_enabled(self, cluster):
        ...
        try:
            ...
            if not ray_cluster:
                print(f"Warning: Could not find Ray cluster {cluster.config.name} in namespace {cluster.config.namespace}")
                return False
            
            # Check for authentication annotations
            annotations = ray_cluster.get("metadata", {}).get("annotations", {})
            auth_annotation = annotations.get("odh.ray.io/secure-trusted-network", "false")
    ...
    def assert_jobsubmit_withlogin(self, cluster):
        ...
        if is_byoidc_cluster:
            ...
            print("BYOIDC authentication is handled differently and requires browser-based OIDC flow")
            ...
            if ray_cluster and ray_cluster.get("status", {}).get("state") == "ready":
                print("✓ Ray cluster is ready and accessible - BYOIDC authentication test passed")
                return
            else:
                cluster_state = ray_cluster.get("status", {}).get("state", "unknown") if ray_cluster else "not found"
                print(f"✗ Ray cluster is not ready or not accessible (state: {cluster_state})")
                ...
                assert False, f"Ray cluster is not accessible via Kubernetes API (state: {cluster_state})"
        
        if not ray_cluster_auth_enabled:
            # If Ray cluster doesn't have authentication enabled, don't send auth headers
            print("Ray cluster authentication is disabled - proceeding without auth headers")
            header = {}
        else:
            ...
            try:
                ...
                print(f"Could not get auth token: {e} - proceeding without auth headers")
                header = {}
