...
def is_byoidc_cluster_detected():
    ...
    try:
        ...
        if os.getenv("BYOIDC_ADMIN_USERNAME") or os.getenv("TEST_USER_USERNAME", "").startswith("odh-"):
            ...
            if "oidcClients" in status and status["oidcClients"]:
                ...
                for client in status["oidcClients"]:
                    if client.get("clientID") == "oc-cli":
                        print("Detected BYOIDC cluster from status.oidcClients (oc-cli)")
                        return True
...
def assert_get_cluster_and_jobsubmit(
    ...
    if is_byoidc_cluster:
        ...
        print("BYOIDC authentication requires browser-based OIDC flow for Ray Dashboard")
        # Just verify cluster is accessible and clean up
        print("✓ Ray cluster retrieved and accessible via Kubernetes API")
        print("Note: Skipping due to known BYOIDC/Ray Dashboard compatibility limitations")
        cluster.down()
        return
...
def detect_authentication_method():
    ...
    if (os.getenv("OCP_ADMIN_USER_USERNAME") and 
        os.getenv("OCP_ADMIN_USER_PASSWORD") and
        os.getenv("TEST_USER_USERNAME") and 
        os.getenv("TEST_USER_PASSWORD") and
        not KUBE_AUTHKIT_AVAILABLE):
...
def detect_stuck_oauth_proxy_serviceaccount(namespace, timeout_minutes=3):
    ...
    try:
        ...
        for event in events.items:
            if event.message and any(pattern in event.message for pattern in stuck_patterns):
                # Check if this event is recent and persistent
                if event.first_timestamp:
                    event_age = datetime.now(event.first_timestamp.tzinfo) - event.first_timestamp
                    if event_age.total_seconds() > (timeout_minutes * 60):
                        ...
                        print(f"   This appears to be a product bug - will attempt cluster recreation")
                        return True
...
def wait_ready_with_stuck_detection(cluster, timeout=600, dashboard_check=True):
    ...
    print(f"🔄 Waiting for cluster {cluster.config.name} to be ready (with stuck detection)...")
    ...
    recovery_attempted = wait_ready_with_stuck_detection._recovery_attempted.get(cluster_key, False)
    ...
    def monitor_for_stuck_resources():
        ...
        if detect_stuck_oauth_proxy_serviceaccount(cluster.config.namespace, timeout_minutes=3):
            ...
            if not recovery_attempted:
                ...
                try:
                    ...
                    wait_ready_with_stuck_detection._recovery_attempted[cluster_key] = True
                    ...
                    print("✅ Cluster recreation completed - normal wait_ready will continue")
    ...
    if not recovery_attempted:
        monitor_thread = threading.Thread(target=monitor_for_stuck_resources, daemon=True)
    ...
    try:
        ...
        if result:
            ...
            print(f"❌ Cluster {cluster.config.name} failed to become ready within timeout")
        return result
...
def setup_token_refresh_monitoring():
    ...
    try:
        ...
        print("Note: kube-authkit handles token refresh automatically - no monitoring needed")
...
def setup_byoidc_user_context(username, password):
    ...
    try:
        ...
        oidc_issuer = f"{issuer_url}/realms/openshift" if '/realms/' not in issuer_url else issuer_url
        ...
        if current_user and current_user.strip() == username:
            print(f"✅ Successfully set up and switched to BYOIDC user context: {username}")
            return True
        else:
            print(f"ERROR: Context switch verification failed. Expected: {username}, Got: {current_user}")
            return False
